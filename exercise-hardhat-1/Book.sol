...
contract Book {
    ...
    event UpdateBook(address indexed sender, string indexed ISBN, string newTitle, string newYear, string newAuthor);
    ...
    function getBook(string calldata _ISBN) external view returns (string memory title, string memory year, string memory author) {
        BookData memory bookData = s_ISBNtoBookData[_ISBN];

        return (bookData.title, bookData.year, bookData.author);
    }

    function addBook(string calldata _title, string calldata _year, string calldata _author) external onlyAdmin {
    ...
    function updateBook(
        ...
        emit UpdateBook(msg.sender, bookData.ISBN, bookData.title, bookData.year, bookData.author);
    }
    ...
    function getISBN(string memory _title, string memory _year, string memory _author) internal view returns (string memory) {
        bytes memory hashData = abi.encodePacked(getBookCount(), _title, _year, _author);
